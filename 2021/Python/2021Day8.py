# -*- coding: utf-8 -*-
"""
Created on Wed Dec  8 05:51:13 2021

@author: choes
"""

input = """gbcead cfgaeb beadf adb egafd fbeac dbfegca fdaceb dbfc bd | dfcb gedaf dcfb bcfdea
...
inputOutputPairs = [(x.split("|")[0], x.split("|")[1]) for x in inputLines]
...
for i, o in inputOutputPairs:
    x = i.strip().split(" ")
    y = list(processString(s) for s in x)
    inputCodes.append(y)
    
    x = o.strip().split(" ")
    y = list(processString(s) for s in x)
    outputCodes.append(y)
    

# Part 1
count = 0
for codes in outputCodes:    
    for code in codes:
...
def decode(inputCodes, outputCodes):
    ...
    # Stage 1 - working out 3 and 6:
    length5Codes = [code for code in inputCodes if len(code) == 5]
    remainingLength5Codes = []
    length6Codes = [code for code in inputCodes if len(code) == 6]
    remainingLength6Codes = []
    
    for code in length5Codes:
        if len(stringIntersect(numberToCode[1], code)) == 2:
            numberToCode[3] = code
            codeToNumber[code] = 3
        else:
            remainingLength5Codes.append(code)
    
    for code in length6Codes:
        if len(stringIntersect(numberToCode[1], code)) == 1:
            numberToCode[6] = code
            codeToNumber[code] = 6
        else:
            remainingLength6Codes.append(code)
            
    # Stage 2: 
    c = stringDifference(numberToCode[1], numberToCode[6])    
    f = stringDifference(numberToCode[1], c)    
    a = stringDifference(numberToCode[7], numberToCode[1])
    
    # Stage 3:
    
    for code in remainingLength5Codes:
        if len(stringIntersect(code, f)) == 1:
            codeToNumber[code] = 5
            numberToCode[5] = code
        else:
            codeToNumber[code] = 2
    ...
    e = stringDifference(numberToCode[6], numberToCode[5])
    
    for code in remainingLength6Codes:
        if len(stringIntersect(code, e)) == 1:
            codeToNumber[code] = 0
            numberToCode[0] = code
        else:
            codeToNumber[code] = 9
    ...
    return int("".join([str(codeToNumber[code]) for code in outputCodes]))
    
    
testInput = "acedgfb cdfbe gcdfa fbcad dab cefabd cdfgeb eafb cagedb ab".split(" ")
testOutput = "cdfeb fcadb cdfeb cdbaf".split(" ")

result = decode([processString(s) for s in testInput], [processString(s) for s in testOutput])


# Part 2:
output = 0
for i in range(len(inputCodes)):
    output += decode(inputCodes[i], outputCodes[i])
print(output)